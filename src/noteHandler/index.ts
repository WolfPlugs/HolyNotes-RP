...
const nativeHandle = FileSystemDirectoryHandle
...
module.exports = class noteHandler {
    ...
    public initNotes = async () => {
      return await noteFiles
    }
  
    public getNotes = async () => {
      const thenoteFiles = await this.initNotes()
      return thenoteFiles.all()
    }
  
    public addNote = async (noteData, notebook) => {
      await this.initNotes()
      let notes
      try { notes = await this.getNotes() }
      catch { return }
